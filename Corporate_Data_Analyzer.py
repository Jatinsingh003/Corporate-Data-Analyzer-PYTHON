...
class CorporateReportBuilder:
    ...
    def _build_ui(self):
        ...
        tk.Button(row2, text="Export Report (Save As)", command=self.export_report, width=20).pack(side="left", padx=5)
    ...
    def export_report(self):
        ...
        if fmt.startswith("Excel"):
            ...
            default_name = "report_output.xlsx"
        else:
            extension = ".csv"
            filetypes = [
                ("CSV Files", "*.csv"),
            ...
            default_name = "report_output.csv"

        # Save As dialog lets the user rename the file and choose the destination folder.
    ...
    def preview_chart(self):
        ...
        try:
            ...
            # A larger figure plus explicit margins prevents titles, labels and
            # percentages from being clipped inside the Tkinter preview.
            ...
            if chart_type in ["Bar", "Column"]:
                ...
                # Pie works best when labels are few. A smaller radius gives
                # labels and percentages enough room inside the preview.
                pie_n = 6 if len(values) > 6 else len(values)
                pie_values = values[:pie_n]
                pie_labels = labels[:pie_n]
                ...
                # Give the pie chart extra room on every side.
                ax.set_position([0.08, 0.08, 0.84, 0.76])

            ax.set_title(
                f"{chart_type} Chart: {data.columns[1]} by {data.columns[0]}",
                pad=16,
                fontsize=12,
            )

            if chart_type != "Pie":
                # Keep rotated labels and the title inside the figure.
                fig.tight_layout(pad=1.8)

            # Extra padding is important for pie labels and for PNG export.
            fig.subplots_adjust(top=0.86, bottom=0.12, left=0.10, right=0.92)
    ...
    def export_chart(self):
        ...
        # Save As dialog lets the user rename the PNG and choose its location.
        out_path = filedialog.asksaveasfilename(
            title="Save Chart As",
            defaultextension=".png",
            initialfile="chart_output.png",
            filetypes=[
                ("PNG Image", "*.png"),
                ("All Files", "*.*"),
            ],
        ...
        try:
            # bbox_inches='tight' + pad_inches prevents the exported image
            # from cutting off the title, labels or other chart text.
